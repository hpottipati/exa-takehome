#!/usr/bin/env python3
"""
Test script to evaluate SERP DuckDuckGo search results using the Authority Scorer
"""
import json
import os
import sys
...
from search.serp_duckduckgo import SerpDuckDuckGoClient
...
def test_single_query(query_data: Dict, client: SerpDuckDuckGoClient) -> Dict:
    """Test a single query and return results"""
    print(f"\n{'='*60}")
    print(f"Testing Query {query_data['id']}: {query_data['scenario'][:50]}...")
    print(f"Search Query: {query_data['search_query']}")
    
    # Get SERP DuckDuckGo search results
    try:
        serp_results = client.search(
            query=query_data['search_query'],
            num_results=10
        )
        print(f"Retrieved {len(serp_results)} results from SERP DuckDuckGo")
    except Exception as e:
        print(f"Error fetching SERP DuckDuckGo results: {e}")
        return None
    ...
    providers_results = {
        'serp_duckduckgo': search_results
    }
    ...
    if 'serp_duckduckgo' in evaluation['providers']:
        metrics = evaluation['providers']['serp_duckduckgo']['metrics']
        print(f"\nMetrics for SERP DuckDuckGo:")
        ...
        print(f"\nTop 3 Results:")
        for result in evaluation['providers']['serp_duckduckgo']['detailed_results'][:3]:
...
def main():
    ...
    # Initialize SERP DuckDuckGo client
    client = SerpDuckDuckGoClient()
    ...
    if all_results:
        print(f"\n{'='*60}")
        print("AGGREGATE REPORT - SERP DUCKDUCKGO")
        print('='*60)
        
        report = generate_authority_report(all_results)
        
        if 'serp_duckduckgo' in report.get('summary', {}):
            summary = report['summary']['serp_duckduckgo']
            print(f"\nSERP DuckDuckGo Search - Average Metrics:")
        ...
        output_file = results_dir / f"serp_duckduckgo_authority_results_{timestamp}.json"
